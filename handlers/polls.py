...
async def polls_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    logger.info(f"User selected in Polls: {user_input}")
    ...
    if user_input == "🔙 Назад":
        ...
        # Тут можна додати логіку для відображення опитувань
        await update.message.reply_text("📊 Функція 'Опитування' ще не реалізована.")
        return States.POLLS
