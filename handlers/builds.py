...
async def builds_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    logger.info(f"User selected in Builds: {user_input}")
    ...
    if user_input == "🔙 Назад":
        ...
        # Тут можна додати логіку для відображення збірок
        await update.message.reply_text("🛠 Функція 'Збірки' ще не реалізована.")
